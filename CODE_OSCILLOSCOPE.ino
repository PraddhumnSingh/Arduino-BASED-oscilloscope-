...
void writeCommonImage() {                 // Common screen image drawing
  ...
  oled.setCursor(88, 0);                  // Start at top-left corner
...
void startScreen() {                      // Staru up screen
  ...
  oled.println(F("MINI~OSCILLOSCOPE"));             //             // Title(Poor Man's Osilloscope, RadioPench 1)
  oled.setTextSize(1);                
  
  oled.setCursor(0, 37);
  oled.println(F("DESIGN BY-"));  
  oled.setCursor(47, 47);
  oled.println(F("ER.PRADDHUMN"));
  oled.setCursor(90, 57);
  oled.println(F("SINGH"));              // version No.
